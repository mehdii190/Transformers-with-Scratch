...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self,d_model: int , seq_len : int , dropout: float) -> None:
        ...
        pe = pe.unsqueeze(0) # (1, seq len , d model)
...
class FeedForwardBlack(nn.Module):
...
class MultHeadAttentionBlock(nn.Module):
    ...
    def forward(self, q, k , v, mask):
        ...
        x , self.attention_scores = MultHeadAttentionBlock.attention(query, key , value, mask, self.dropout)
...
def build_transformer(src_vocab_size: int,
    ...
    for _ in range(N):
        ...
        encoder_self_attention_block = MultHeadAttentionBlock(d_model, h, dropout)
        feed_forward_block = FeedForwardBlack(d_model,d_ff,dropout)
    ...
    for _ in range(N):
        ...
        decoder_self_attention_block = MultHeadAttentionBlock(d_model, h, dropout)
        decoder_cross_attention_block = MultHeadAttentionBlock(d_model, h, dropout)
        feed_forward_block = FeedForwardBlack(d_model,d_ff,dropout)
